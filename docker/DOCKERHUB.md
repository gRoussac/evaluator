...
docker run --rm -p 4000:4000 -p 9788:9788 interchouette/evaluator:dev
```

Default: **web on :4000** + **MCP HTTP on :9788**. Opt out with `ENABLE_MCP=0`.

Prefer **MCP HTTP** (`http://localhost:9788/mcp` locally, or `https://mcp:mcp@host/mcp` via the web gateway). Stdio pays a Docker spawn cost on cold start.
...
| Invocation | Behavior |
| --- | --- |
| *(default / `web`)* | Web UI + API on `:4000`; MCP HTTP sidecar on `:9788` if `ENABLE_MCP=1` |
...
| `ENABLE_MCP_PROXY` | `1` | Express proxies `/mcp` → sidecar `:9788` (basic auth) |
...
| `EVALUATOR_MCP_ADDR` | `0.0.0.0:9788` | MCP HTTP bind |
...
Platforms like Render expose only `PORT` (web). The sidecar still listens on `:9788` **inside** the container; it is **not** `https://host:9788/mcp`.
...
AI clients: Streamable HTTP at `http://localhost:9788/mcp` (local sidecar, no auth) or `https://mcp:mcp@<host>/mcp` (proxied). Stdio: spawn `… mcp`. Tools: `evaluate`, `list_functions`, `batch` (Rust `evaluator-mcp` → Node evaluate/batch).
