...
var require_format_error = __commonJS({
  "../node_modules/sqlite/build/utils/format-error.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
...
var require_Statement = __commonJS({
  "../node_modules/sqlite/build/Statement.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
...
var require_migrate = __commonJS({
  "../node_modules/sqlite/build/utils/migrate.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    ...
    async function readMigrations(migrationPath) {
      ...
      const migrationFiles = await new Promise((resolve3, reject) => {
        fs.readdir(location, (err, files) => {
          if (err) {
            return reject(err);
          }
          resolve3(files.map((x) => x.match(/^(\d+).(.*?)\.sql$/)).filter((x) => x !== null).map((x) => ({ id: Number(x[1]), name: x[2], filename: x[0] })).sort((a, b) => Math.sign(a.id - b.id)));
        });
      });
      if (!migrationFiles.length) {
        throw new Error(`No migration files found in '${location}'.`);
      }
      return Promise.all(migrationFiles.map((migration) => new Promise((resolve3, reject) => {
...
var require_strings = __commonJS({
  "../node_modules/sqlite/build/utils/strings.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
...
var require_Database = __commonJS({
  "../node_modules/sqlite/build/Database.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
      ...
      prepare(sql, ...params) {
        return new Promise((resolve3, reject) => {
          ...
          const stmt = this.db.prepare(sqlObj.sql, ...sqlObj.params, (err) => {
            ...
            resolve3(new Statement_1.Statement(stmt));
          });
        });
      }
      /**
    ...
    exports2.Database = Database4;
  }
});

// apps/evaluator/server.ts
var server_exports = {};
...
// libs/util/puppeteer/src/lib/browser-engine.ts
var CHROMIUM_LAUNCH_ARGS = [
  "--no-sandbox",
  "--disable-setuid-sandbox",
  "--disable-dev-shm-usage"
];
var EVALUATE_USER_AGENT = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.0.0 Safari/537.36";
var EVALUATE_TIMEOUT_MS = 24e4;
function chromiumExecutablePath() {
  return process.env["PUPPETEER_EXECUTABLE_PATH"] || void 0;
}
function usePuppeteer() {
  const v = process.env["USE_PUPPETEER"];
  return v === "1" || v === "true" || v === "TRUE";
}
function isValidHttpUrl(url_test) {
  let url;
  try {
...
// libs/util/puppeteer/src/lib/playwright.engine.ts
var import_playwright_core = require("playwright-core");
var import_rxjs = __toESM(require_cjs());

// libs/util/sqlite/src/lib/sqlite.service.ts
...
async function open(config2) {
  const db = new import_Database.default.Database(config2);
  await db.open();
  return db;
}
...
var DEFAULT_DB_PATH = (0, import_path.resolve)(
  process.env["SQLITE_PATH"] || "db/database.db"
);
var SqliteService = class {
  async open() {
    (0, import_fs.mkdirSync)((0, import_path.dirname)(DEFAULT_DB_PATH), { recursive: true });
    this.db = await open({
...
// libs/util/puppeteer/src/lib/eval.template.ts
var START = "##START##";
var test = `\`${START}\${ stringify_back(expr, (key, val) => typeof val === 'function' ? val.toString().replace(/\\s+/gm, ' ') : val) }\``;
var template = `
...
`;
var eval_template_default = template;
...
function resolveFunctionsPath() {
  ...
  if (fromEnv) {
    return (0, import_path2.resolve)(fromEnv);
  }
  const sqlite = process.env["SQLITE_PATH"]?.trim();
  if (sqlite) {
    return (0, import_path2.join)((0, import_path2.dirname)((0, import_path2.resolve)(sqlite)), "functions.json");
  }
  return (0, import_path2.resolve)(process.cwd(), "db", "functions.json");
}
...
function resolveFunctionExpression(fnSha256) {
  const functions = readFunctionsCatalog();
  if (!functions || !fnSha256) {
    return "";
  }
  let fnGroup = "";
  const func = Object.values(functions).map((group, index) => {
    const groupfound = group.find((fn) => fn.sha256 === fnSha256);
    if (groupfound) {
...
// libs/util/puppeteer/src/lib/resolve-fn.ts
var regeXss = /[\w]+\.[\w]+(\.[\w]+)?/;
function buildEvalTemplate(message) {
  let tpl = eval_template_default;
  let fn = "";
  ...
  if (fn) {
    tpl = eval_template_default.replace(/window.eval/gm, fn);
  }
  return tpl;
}
...
var PlaywrightEngine = class {
  constructor(ws) {
    this.ws = ws;
    this.result$ = new import_rxjs.Subject();
  ...
  async goto(message, opts) {
    ...
    try {
      ...
      await page.route("**/*", async (route) => {
        ...
        if (req.isNavigationRequest() && req.frame() === page.mainFrame() && !req.url().includes(hostname)) {
          aborted = true;
          url = req.url();
      ...
      let error = false;
      await page.goto(message.url.trim(), {
      ...
      await page.context().close().catch(() => void 0);
    }
  }
  async getNewPage(message) {
...
// libs/util/puppeteer/src/lib/puppeteer.engine.ts
var puppeteer = __toESM(require("puppeteer"));
var import_rxjs2 = __toESM(require_cjs());
var PuppeteerEngine = class {
  constructor(ws) {
    this.ws = ws;
    this.result$ = new import_rxjs2.Subject();
    this.sqliteService = new SqliteService();
    this.browser = this.getBrowser();
  }
  async getBrowser() {
  ...
  async goto(message, opts) {
    ...
    const takeScreenshot = opts?.screenshot !== false;
    const waitUntil = opts?.waitUntil === "load" ? ["load"] : opts?.waitUntil === "domcontentloaded" || opts?.waitUntil === "commit" ? ["domcontentloaded"] : opts?.waitUntil === "networkidle" ? ["domcontentloaded", "networkidle0"] : ["domcontentloaded", "networkidle0"];
    try {
      this.setListener(page);
      let aborted = false;
      let url = "";
...
// libs/util/puppeteer/src/lib/engine.factory.ts
function createEvaluateSession(ws) {
  if (usePuppeteer()) {
...
// libs/util/puppeteer/src/lib/run-evaluate.ts
var Crypto = __toESM(require("crypto"));
var import_rxjs3 = __toESM(require_cjs());
var RESULT_MAX = 1e3;
function dedupAndFilter() {
  const duplicates = /* @__PURE__ */ new Map();
  return (0, import_rxjs3.pipe)(
    (0, import_rxjs3.map)((message) => {
    ...
    (0, import_rxjs3.filter)((x) => !!x?.sha256),
    (0, import_rxjs3.take)(RESULT_MAX)
  );
}
function decorateResult(message) {
  const result = JSON.parse(message.text.replace(START, "").trim()).map(
    (entry) => typeof entry === "string" ? entry.trim().replace(/\n/g, " ").replace(/\s\s+/g, " ") : entry
  );
  const sha256 = Crypto.createHash("sha256").update(message.text).digest("hex");
  let stacktrace = message.stackTrace.map((trace) => ({ ...trace }));
  const lastcaller = stacktrace.length ? stacktrace.slice(-1)[0] : void 0;
  ...
  const firstcaller = stacktrace.length ? stacktrace[0] : void 0;
...
async function runEvaluate(opts) {
  ...
  const message = { url, fn, clearFn };
  const session = createEvaluateSession(opts.ws);
  const results = [];
  const subscription = session.results.pipe(dedupAndFilter()).subscribe((result) => {
    if (!result) return;
    results.push(result);
  ...
  try {
    ...
    return { results, screenshot: screenshot || void 0 };
  } finally {
...
async function runBatch(opts) {
  ...
  try {
    for (const url of urls) {
      ...
      const outcome = takeScreenshot ? { results, screenshot } : { results };
      count += 1;
...
// libs/util/puppeteer/src/lib/puppeteer.service.ts
var PuppeteerResolver = class _PuppeteerResolver {
  static {
    this.url_not_valid = "not a valid url?";
  }
  static {
    this.parse_failure = "parsing failure";
  }
  static async resolve(req, res, next) {
    ...
    const clearFn = !!fn;
    if (!isValidHttpUrl(url)) {
      res.status(400).send([_PuppeteerResolver.url_not_valid]);
      return;
    }
    res.write("[");
    try {
      ...
      res.status(500).send([_PuppeteerResolver.parse_failure, error?.toString()]);
      return next(error);
    }
  }
  static async resolveWs(message, ws) {
...
var user = process.env["DB_USER"] || "user";
var pwd = process.env["DB_PWD"] || "pwd";
var users = { [user]: pwd };
var auth = (0, import_express_basic_auth.default)({
  users,
  challenge: true
});
var mcpUser = process.env["MCP_USER"] || "mcp";
var mcpPwd = process.env["MCP_PWD"] || "mcp";
var mcpAuth = (0, import_express_basic_auth.default)({
...
function proxyOnError(label) {
  return (err, _req, res) => {
    ...
    if (res && typeof res === "object" && "writeHead" in res && typeof res.writeHead === "function" && "headersSent" in res && !res.headersSent) {
      res.writeHead(502, {
...
var apiProxy = (0, import_http_proxy_middleware.createProxyMiddleware)({
...
var mcpProxy = (0, import_http_proxy_middleware.createProxyMiddleware)({
  target: "http://127.0.0.1:9790",
  changeOrigin: true,
...
function createApp() {
  ...
  const distFolder = (0, import_path3.join)(process.cwd(), "dist/evaluator/browser");
  app.get(/^\/evaluate(\/.*)?$/, PuppeteerResolver.resolve);
  app.use("/db", auth);
  app.get("/db/database.db", (_req, res) => {
    const sqlitePath = process.env["SQLITE_PATH"]?.trim() || (0, import_path3.join)(process.cwd(), "db", "database.db");
  ...
  app.use(import_express.default.static(distFolder, { maxAge: "1y", index: false }));
  app.get("/{*path}", (_req, res) => {
    res.sendFile((0, import_path3.join)(distFolder, "index.html"));
  });
  return app;
}
function attachWebSocket(server) {
  ...
  wss.on("connection", (ws) => {
    ...
    ws.on("message", async (message_raw) => {
      ...
      const message = JSON.parse(message_raw.toString());
...
function normalizeSite(raw) {
  ...
  if (site.includes(".") || site.startsWith("localhost") || site.startsWith("127.")) {
    return `https://${site}`;
  }
  return null;
}
function loadCsvUrls(path) {
  const text = (0, import_fs3.readFileSync)(path, "utf8");
  const lines = text.split(/\r?\n/).filter((l) => l.trim().length > 0);
  if (lines.length === 0) return [];
  const splitCols = (line) => line.split(",").map((c) => c.trim().replace(/^["']|["']$/g, ""));
...
async function runEvaluateCli(argv) {
...
async function runBatchCli(argv) {
