...
export class PuppeteerEngine implements EvaluateSession {
  ...
  async goto(
    ...
    // Default matches historical UI/evaluate behavior (domcontentloaded + networkidle0).
    // Puppeteer has no 'commit'; map it to 'domcontentloaded'.
    const waitUntil: Array<'load' | 'domcontentloaded' | 'networkidle0'> =
      opts?.waitUntil === 'load'
        ? ['load']
        : opts?.waitUntil === 'domcontentloaded' || opts?.waitUntil === 'commit'
          ? ['domcontentloaded']
